...
Концепция идеальной формы организации информации, собранной из лучших сторон всех исторических форм с устранением их минусов: **«GitHub для знаний», скрещённый с Википедией и форумом**. Комплект из 26 документов:
...
| 24 | [Глазами машины](zhivaya-biblioteka/24-glazami-mashiny.md) | ИИ-библиотекарь изнутри: железное правило §10.4, дисциплина источников, машина без репутации и власти, 6 абсолютных «никогда» |
| 25 | [Глазами второго инстанса](zhivaya-biblioteka/25-glazami-vtorogo-instansa.md) | Федерация изнутри: зачем свой инстанс, подписка и форк на практике, смерть соседа, дипломатия малого |
...
### ⚙️ [prototype/](prototype/) — рабочий прототип (фазы 0–4 + доверие, подписи Ed25519, годовой контур, апелляции, форк)
...
- [fork.py](prototype/fork.py) — **форк рубрики из реплики в свой канон** (§17.5, §25.4): только из `replicas/` (нет подписки — нет форка), паспорт `forked_from {instance, card, version, url}`, локальная версия с 1, реплика-баннер срезается, рубрика добавляется в `_rubrics.yml` целевого инстанса, `rubric.forked` подписано ключом локального хранителя (`edsig=`); конфликт id → отказ («решают люди»)
...
- [ed25519.py](prototype/ed25519.py) — **Ed25519 на чистом Python** (RFC 8032, без зависимостей, проверен на тест-векторах RFC): keygen/sign/verify
- [sign.py](prototype/sign.py) — **ключи и подписи журнала v2** (§17.5): асимметричные подписи `edsig=` (Ed25519) — **публичные ключи в [_keys/registry.yml](prototype/_keys/registry.yml), проверка не требует секрета**; legacy HMAC `sig=` остаётся валидным (журнал append-only, старое не переподписывается); `verify-ledger` ловит подделку обоих форматов; приватные seed не коммитятся
...
- [ci-freshness.yml](prototype/ci-freshness.yml) — шаблон workflow: еженедельный cron-сканер + deadman-алерт. **Активация в `.github/workflows/` заблокирована областью токена** (нет `workflow`-скопа): чтобы включить CI, положите этот файл в `.github/workflows/` вручную
...
- [_ledger.log](prototype/_ledger.log) — публичный журнал власти (append-only, **56 событий**; действия ролей подписаны: 6 Ed25519 + 14 legacy HMAC, все валидны)
...
- [dist/](prototype/dist/) — собранный сайт: канон, журнал власти (56 событий), здоровье, черновики, RSS-фиды, федерация (8 Articles)
...
свои хранители (daria_v, oleg_t), подписка на рубрики альфы через subscribe.py — реплики read-only в
...
**Форк рубрики (§17.5, §25.4) отработан end-to-end:** рубрика `formy/katalogi` забрана из реплики в
свой канон — [kn-2026-0007](prototype-beta/canon/formy/katalogi/kn-2026-0007.md) с паспортом
`forked_from: lib-alpha v9` и локальной версией 1; событие `rubric.forked` (ldg-0001) подписано
Ed25519-ключом daria_v@lib-beta и проверяется публично по [_keys/registry.yml](prototype-beta/_keys/registry.yml).
Отказы показаны все: без подписки, без рубрики, без ключа, не-хранителю, при конфликте id.
Свой журнал ([_ledger.log](prototype-beta/_ledger.log)), свои ключи, свои рубрики — канон федеративен, власть локальна.
